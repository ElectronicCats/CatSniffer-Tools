...
class HCICommandDispatcher:
    ...
    def handle_disconnect(self, opcode, params):
        ...
        if len(params) >= 3:
            handle = struct.unpack('<H', params[:2])[0]
            reason = params[2]
            self.bridge.log.info("Disconnect handle=0x%04X reason=0x%02X", handle, reason)
    ...
    def handle_le_set_advertising_parameters(self, opcode, params):
        ...
        if len(params) >= 15:
            ...
            self.bridge.log.debug("Adv params: interval=%d type=%d", self.bridge.adv_interval, adv_type)
        return events.command_complete(opcode, bytes([0x00]))
    ...
    def handle_le_set_scan_parameters(self, opcode, params):
        ...
        if len(params) >= 7:
            ...
            self.bridge.log.debug("Scan params: type=%d interval=%d window=%d",
    ...
    def handle_le_create_connection(self, opcode, params):
        ...
        if len(params) >= 25:
            ...
            # Initiate connection
            self.bridge.initiate_connection(peer_addr, peer_addr_type,
    ...
    def handle_le_read_channel_map(self, opcode, params):
        ...
        if len(params) >= 2:
            ...
            chan_map = getattr(self.bridge, 'channel_map', bytes([0xFF, 0xFF, 0x1F, 0x00, 0x00]))
            
            # Response: status(1) + handle(2) + channel_map(5)
            data = bytes([0x00]) + struct.pack('<H', handle) + chan_map
            self.bridge.log.debug("Read Channel Map: handle=0x%04X map=%s", handle, chan_map.hex())
            return events.command_complete(opcode, data)
    ...
    def handle_le_set_data_length(self, opcode, params):
        ...
        if len(params) >= 6:
            ...
            self.bridge.log.debug("Set data length: handle=0x%04X octets=%d time=%d",
                                  handle, tx_octets, tx_time)
        return events.command_complete(opcode, bytes([0x00]))
