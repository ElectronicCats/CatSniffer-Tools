...
class SpectrumScan:
    ...
    def __load_parser(self):
        self.parser.add_argument("port", type=str, help="COM port to connect to the device", default=DEFAULT_COMPORT)
        self.parser.add_argument("-b", "--baudrate", type=int, help=f'COM port baudrate (defaults to {DEFAULT_BAUDRATE})', default=DEFAULT_BAUDRATE)
        self.parser.add_argument("--freqStart", type=float, help=f"Starting frequency in MHz (Default to {DEFAULT_START_FREQ})", default=DEFAULT_START_FREQ)
        self.parser.add_argument("--freqEnd", type=float, help=f"End frequency in MHz (Default to {DEFAULT_END_FREQ})", default=DEFAULT_END_FREQ)
        self.parser.add_argument("--offset", type=int, help=f"Default RSSI offset in dBm (defaults to {DEFAULT_RSSI_OFFSET})", default=DEFAULT_RSSI_OFFSET)

    def __data_dissector(self, plot_data):
        if FREQ_FRAME_MARK in plot_data:
            self.current_freq = float(plot_data.split(' ')[1])
            if self.current_freq >= self.start_freq and self.current_freq <= self.end_freq:
                if self.current_freq == self.start_freq:
                    self.data_matrix = np.zeros((SCAN_WIDTH, self.delta_freq))
                return
        if (START_OF_FRAME in plot_data) and (END_OF_FRAME in plot_data):
            if self.current_freq >= self.start_freq and self.current_freq <= self.end_freq:
                scan_line = plot_data[len(START_OF_FRAME):-len(END_OF_FRAME)].split(",")[:-1]
                data = list(map(int, scan_line))
                index = int((self.current_freq - self.start_freq) / DEFAULT_STEP_PER_FREQ)
    ...
    def create_plot(self):
        ...
        self.fig.suptitle(f"SX126x Spectral Scan (Frequency range: {self.start_freq}/{self.end_freq} MHz)")
        self.fig.canvas.manager.set_window_title("PWNLabs/ElectroniCats -  Spectral Scan")
        self.im = self.ax.imshow(
            self.data_matrix[:,:self.delta_freq],
            cmap=DEFAULT_COLOR_MAP,
            aspect="auto",
            extent=[self.start_freq, self.end_freq, -4 *(SCAN_WIDTH+1), self.rssi_offset],
        )
    ...
    def main(self):
        ...
        ani = animation.FuncAnimation(self.fig, self.show_plot, interval=100, cache_frame_data=False)
        plt.show()
        sys.exit(0)
