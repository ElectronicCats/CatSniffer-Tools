...
class Graphs:
    ...
    def generate_topology_graph(self):
        self.table = Table(
            title=f"Network Topology - {0x0000}",
    ...
    def generate_channel_graph(self):
        ...
        for channel, data in self.channel_activity.items():
            ...
            self.table.add_row(
                ...
                str(self.channel_activity[channel]),
            )
        return self.table
    ...
    def create_channel_graph(self):
        with Live(
            self.generate_channel_graph(), refresh_per_second=REFRESH_INTERVAL
        ) as live:
            while self.running:
                time.sleep(0.4)
    ...
    def create_topology_graph(self):
        with Live(
            self.generate_topology_graph(), refresh_per_second=REFRESH_INTERVAL
        ) as live:
            while self.running:
                time.sleep(0.4)
